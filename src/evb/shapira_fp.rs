...
pub fn calculate_xshap(x1: f64, x2: f64, params: &FocalPlaneTilt) -> Option<f64> {
    let dx = x2 - x1;
    ...
    let h_mm = params.h * params.s;
    let numerator = x2 * (params.s / norm_tan) - (dx * h_mm);

    let denominator = (params.s / norm_tan) - (dx / norm_cot);
